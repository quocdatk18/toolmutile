            ...
            if (category === 'accOkvip') {
                ...
                while (!registrationSuccess && retryCount < maxRetries) {
                    ...
                    try {
                        ...
                        // Check for error message (phone already registered)
                        const errorDetected = await currentPage.evaluate(() => {
                            // Look for error messages in specific error containers
                            const errorContainers = document.querySelectorAll('[class*="error"], [class*="alert"], [class*="message"], .van-toast, .van-notify');
                            ...
                            for (const container of errorContainers) {
                                ...
                                for (const keyword of errorKeywords) {
                                    if (text.includes(keyword)) {
                                        console.log(`🔴 Error message found: ${container.textContent}`);
                                        return true;
                                    }
                                }
                            }
                        ...
                        if (errorDetected) {
                            ...
                            if (retryCount < maxRetries && viotpToken) {
                                console.log(`🔄 Retrying with new phone number on new tab...`);
                                ...
                                if (newPhoneResult && newPhoneResult.phoneNumber) {
                                    ...
                                    // Open new tab instead of trying to fix current form
                                    ...
                                    // Close old page and use new page
                                    await currentPage.close();
                                    currentPage = newPage;

                                    // Continue with "Gửi đi" on new page
                                    continue;
                                } else {
