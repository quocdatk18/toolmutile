...
async function saveGlobalApiKey() {
    ...
    if (result.success) {
        showToast('success', 'Đã lưu', 'API Key đã được lưu. Đang kiểm tra...');
        loadApiKeyInfo();
...
async function checkGlobalBalance() {
    ...
    balanceElement.textContent = '⏳ Đang kiểm tra...';
...
async function saveSimApiKey() {
    ...
    if (result.success) {
        showToast('success', 'Đã lưu', 'SIM API Key đã được lưu thành công');
        loadSimApiKeyInfo();
...
function toggleProxyFields() {
    ...
    proxyFields.style.display = useProxy ? 'block' : 'none';
}
...
function toggleAdvancedSettings() {
    ...
    advancedFields.style.display = showAdvanced ? 'block' : 'none';
}
...
async function createProfileFromModal() {
    ...
    if (showAdvanced) {
        const resolution = document.getElementById('profileResolution').value;
        const timezone = document.getElementById('profileTimezone').value;
        const cpu = document.getElementById('profileCPU').value;
        const ram = document.getElementById('profileRAM').value;
        const canvas = document.getElementById('profileCanvas').value;
        const webrtc = document.getElementById('profileWebRTC').value;

        // Only add if user selected (not auto)
        if (resolution) config.resolution = resolution;
        if (timezone) config.timezone = timezone;
        if (cpu) config.hardwareConcurrency = parseInt(cpu);
        if (ram) config.deviceMemory = parseInt(ram);
        if (canvas) config.canvas = canvas;
        if (webrtc) config.webrtc = webrtc;
    }
    ...
    if (useProxy) {
        ...
        if (!proxyHost || !proxyPort) {
            showToast('warning', 'Thiếu thông tin', 'Vui lòng nhập đầy đủ Proxy Host và Port');
            return;
        }
