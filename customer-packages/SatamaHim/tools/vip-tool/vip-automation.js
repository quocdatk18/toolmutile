...
class VIPAutomation {
    ...
    async registerStep(browser, category, siteConfig, profileData) {
        ...
        try {
            ...
            // For OKVIP OTP: Solve Geetest V4 captcha via AutoCaptcha
            if (category === 'okvipOtp') {
                console.log(`🔐 OKVIP OTP: Solving Geetest V4 captcha...`);

                const apiKey = this.settings?.captchaApiKey || process.env.CAPTCHA_API_KEY;
                if (!apiKey) {
                    console.warn('⚠️ No captcha API key provided, waiting for manual solve...');
                    console.log(`⏳ Please solve the captcha manually...`);

                    let captchaSolved = false;
                    let waitAttempts = 0;
                    const maxWaitAttempts = 300; // Wait up to 5 minutes
                    ...
                    while (!captchaSolved && waitAttempts < maxWaitAttempts) {
                        ...
                        if (successDetected) {
                            console.log('✅ Success detected - Captcha solved');
                            captchaSolved = true;
                            break;
                        }

                        if (waitAttempts % 30 === 0) {
                            console.log(`⏳ Waiting for captcha... (${Math.floor(waitAttempts / 60)} minutes)`);
                        }
                    }

                    if (!captchaSolved) {
                        console.warn('⚠️ Timeout waiting for captcha (5 minutes)');
                        return { success: false, error: 'Timeout waiting for captcha' };
                    }
                } else {
                    // Solve Geetest V4 or Botion captcha via AutoCaptcha
                    const captchaResult = await this.solveGeetestV4ViaAutoCaptcha(page, apiKey);

                    if (captchaResult && captchaResult.success) {
                        // Handle Botion captcha (already solved and dragged in solveBottionCaptcha)
                        if (captchaResult.type === 'botion') {
                            console.log('✅ Botion captcha already solved and dragged');
                        }
                        // Handle Geetest V4 captcha
                        else if (captchaResult.type === 'geetest') {
                            console.log('📝 Filling Geetest V4 response into form...');
                            const fillResult = await page.evaluate((result) => {
                                const challengeField = document.querySelector('input[name*="challenge"], input[name*="geetest_challenge"]');
                                const validateField = document.querySelector('input[name*="validate"], input[name*="geetest_validate"]');
                                const seccodeField = document.querySelector('input[name*="seccode"], input[name*="geetest_seccode"]');

                                if (challengeField) {
                                    challengeField.value = result.challenge;
                                    challengeField.dispatchEvent(new Event('input', { bubbles: true }));
                                }
                                if (validateField) {
                                    validateField.value = result.validate;
                                    validateField.dispatchEvent(new Event('input', { bubbles: true }));
                                }
                                if (seccodeField) {
                                    seccodeField.value = result.seccode;
                                    seccodeField.dispatchEvent(new Event('input', { bubbles: true }));
                                }
                                return { challengeFilled: !!challengeField, validateFilled: !!validateField, seccodeFilled: !!seccodeField };
                            }, captchaResult);
                            console.log('✅ Geetest V4 response filled:', fillResult);
                        }
                    } else {
                        console.error('❌ Failed to solve captcha');
                    }

                    // Step 2: Wait for page to process and redirect (dù captcha thành công hay thất bại)
                    // Vì Botion có thể giải sai nhưng vẫn submit được
                    console.log('⏳ Waiting for page to process captcha response...');
                    let captchaSolved = false;
                    let waitAttempts = 0;
                    ...
                    while (!captchaSolved && waitAttempts < maxWaitAttempts) {
                        ...
                        const currentUrl = page.url();
                        if (!currentUrl.includes('register') && !currentUrl.includes('home')) {
                            console.log(`✅ URL changed to: ${currentUrl} - Registration successful`);
                            captchaSolved = true;
                            break;
                        }
                        ...
                        if (successDetected) {
                            console.log('✅ Success detected - Registration completed');
                            captchaSolved = true;
                            break;
                        }

                        if (waitAttempts % 10 === 0) {
                            console.log(`⏳ Waiting for registration to complete... (${waitAttempts}s)`);
                        }
                    }

                    if (!captchaSolved) {
                        console.warn('⚠️ Timeout waiting for registration to complete');
                        return { success: false, error: 'Timeout waiting for registration to complete' };
                    }
                }

                console.log('✅ OKVIP OTP registration completed (Geetest V4 solved)');
            }
