...
class VIPAutomation {
    ...
    async runSitesParallel(browser, category, sites, profileData, mode, sharedPromoContext, parallelCount, results) {
        ...
        for (let i = 0; i < sites.length; i += parallelCount) {
            ...
            const batchPromises = batch.map(siteName => this.processSite(browser, category, siteName, profileData, mode, sharedPromoContext));
    ...
    async fillOKVIPRegisterForm(page, profileData) {
        // Wait for form fields to appear
        try {
            await page.waitForSelector('input[formcontrolname="account"]', { timeout: 5000 });
