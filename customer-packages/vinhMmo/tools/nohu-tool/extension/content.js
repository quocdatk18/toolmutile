...
// Disabled: showNotification removed to avoid multiple notifications from parallel tabs
...
async function solveAudioCaptchaAuto(audioUrl) {
  ...
  // IMPORTANT: Skip if in checkPromo mode (checkPromotion handler will handle it)
  if (window.isCheckingPromo) {
    console.log('⏭️ Skipping solveAudioCaptchaAuto - checkPromotion handler will handle captcha');
    return;
  }
