...
class OptimizedToolsManager {
    ...
    async processMixedSites(sites, userData, options = {}) {
        console.log('🚀 Starting mixed sites processing...');
        ...
        console.log(`📊 Distribution: NOHU=${nohuSites.length}, SMS=${smsSites.length}`);
        ...
        if (nohuSites.length > 0) {
            console.log('🎯 Processing NOHU sites:', nohuSites.map(s => s.name).join(', '));
            promises.push(
        ...
        if (smsSites.length > 0) {
            console.log('📱 Processing SMS sites:', smsSites.map(s => s.name).join(', '));
            promises.push(
    ...
    async processSingleSite(site, userData, options = {}) {
        ...
        console.log(`🎯 Processing ${site.name} using ${type.toUpperCase()} tool`);
    ...
    async processSequentially(sites, userData, options) {
        console.log('🔄 Processing sites sequentially...');
    ...
    // Cleanup resources
    async cleanup() {
        console.log('🧹 Cleaning up optimized tools...');
