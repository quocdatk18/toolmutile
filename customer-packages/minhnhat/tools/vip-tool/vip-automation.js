...
class VIPAutomation {
    ...
    async solveCaptchaViaAPI(base64Image, apiKey) {
        try {
            ...
            // Detect service type from apiKey format or env variable
            const captchaService = process.env.CAPTCHA_SERVICE || 'autocaptcha'; // Default to autocaptcha

            if (captchaService === 'deathbycaptcha' || apiKey.includes(':')) {
                // DeathByCaptcha format: username:password
                return await this.solveCaptchaViaDeathByCaptcha(base64Image, apiKey);
            } else {
                // AutoCaptcha.pro format
    ...
    async solveCaptchaViaDeathByCaptcha(base64Image, apiKey) {
        try {
            console.log('🔐 Solving captcha via DeathByCaptcha API...');
            console.log('📊 Base64 length:', base64Image?.length || 0);

            // Parse credentials (format: username:password)
            const [username, password] = apiKey.split(':');
            if (!username || !password) {
                console.error('❌ Invalid DeathByCaptcha credentials format (expected: username:password)');
                return null;
            }
            ...
            // Step 1: Submit captcha
            console.log('📤 Sending to DeathByCaptcha API...');
            const submitResponse = await fetch('http://deathbycaptcha.com/api/captcha', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    username: username,
                    password: password,
                    captchafile: `base64:${cleanBase64}`,
                    type: 4 // Image recognition type
                })
            });
            ...
            // Handle error
            if (submitData.error) {
                console.error('❌ Failed to submit captcha:', submitData.error);
                return null;
            }
            ...
            if (!captchaId) {
                console.error('❌ No captcha ID returned');
                return null;
            }
            ...
            for (let i = 0; i < 60; i++) {
                ...
                const resultResponse = await fetch(`http://deathbycaptcha.com/api/captcha/${captchaId}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        username: username,
                ...
                // Check if solved (is_correct: true means solved)
                if (resultData.is_correct) {
                    console.log(`✅ Captcha solved: ${resultData.text}`);
                    return resultData.text;
                }

                // Check for errors
                if (resultData.error) {
                    console.error('❌ Error checking captcha:', resultData.error);
                    return null;
                }
            ...
            console.error('❌ DeathByCaptcha API error:', error.message);
            return null;
        }
    }
    ...
    async registerStep(browser, category, siteConfig, profileData) {
        ...
        try {
            ...
            // Skip captcha for JUN88V2, 22VIP, and AccOKVIP (no captcha after form fill)
            const shouldSolveCaptcha = !['jun88v2', '22vip', 'accOkvip'].includes(category);
            ...
            if (apiKey && shouldSolveCaptcha) {
                ...
                console.log('⏭️ Skipping captcha for JUN88V2, 22VIP, and AccOKVIP (no captcha after form fill)');
            } else {
