...
class VIPAutomation {
    ...
    async fill78WINRegisterForm(page, profileData) {
        try {
            ...
            // Field 3: Full name (firstname) - slow typing
            ...
            // Field 4: Mobile - slow typing with country code handling
            console.log('📱 Filling mobile...');
            let phone = profileData.phone || '';
            if (phone.startsWith('0')) {
                phone = phone.substring(1);
            }

            // Try multiple selectors for mobile input (react-tel-input)
            ...
            for (const selector of mobileSelectors) {
                try {
                    ...
                    if (element) {
                        ...
                        await new Promise(r => setTimeout(r, 300));
                        await page.type(selector, phone, { delay: 150 });
                        mobileFilled = true;
                        console.log(`✅ Mobile filled via: ${selector}`);
                        break;
                    }
                } catch (e) {
                    // Try next selector
            ...
            if (!mobileFilled) {
                console.warn('⚠️ Could not fill mobile field');
            }

            await new Promise(r => setTimeout(r, 800));

            // Field 5: Agree checkbox - check if already checked
    ...
    async fillJUN88V2RegisterForm(page, profileData) {
        try {
            ...
            // Wait for form to be interactive
            await page.waitForSelector('input[id="fullname"]', { timeout: 10000 }).catch(() => null);
            ...
            // Field 1: Full name (fullname) - slow typing
            console.log('👤 Filling full name...');
            await page.focus('input[id="fullname"]');
            await new Promise(r => setTimeout(r, 500));
            await page.type('input[id="fullname"]', profileData.fullname || '', { delay: 120 });
            await new Promise(r => setTimeout(r, 1200));

            // Field 2: Username (username) - slow typing
            console.log('📝 Filling username...');
            await page.focus('input[id="username"]');
            await new Promise(r => setTimeout(r, 500));
            await page.type('input[id="username"]', profileData.username, { delay: 150 });
            await new Promise(r => setTimeout(r, 1200));

            // Field 3: Password (password) - slow typing
            console.log('🔐 Filling password...');
            await page.focus('input[id="password"]');
            await new Promise(r => setTimeout(r, 500));
            await page.type('input[id="password"]', profileData.password, { delay: 150 });
    ...
    async fillJUN88RegisterForm(page, profileData) {
        try {
            ...
            // Field 4: Email - slow typing
            console.log('� Fiilling email...');
            await page.focus('input[id="email"]');
            await new Promise(r => setTimeout(r, 300));
            await page.type('input[id="email"]', profileData.email || '', { delay: 100 });
            await new Promise(r => setTimeout(r, 800));

            // Field 5: Mobile - slow typing with country code handling
            console.log('📱 Filling mobile...');
            let phone = profileData.phone || '';
            if (phone.startsWith('0')) {
                phone = phone.substring(1);
            ...
            // Field 6: Agree checkbox - skip if already checked
            console.log('✅ Checking agree checkbox...');
            try {
                const isChecked = await page.evaluate(() => {
                    const checkbox = document.querySelector('input[id="agree"]');
                ...
                console.warn('⚠️ Could not interact with agree checkbox:', error.message);
                // Continue anyway - checkbox might be auto-checked
            }
