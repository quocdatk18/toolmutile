...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'autoFill') {
    ...
    startAutoFill(request.data, sendResponse);
    return true;
  }
...
function startAutoFill(data, sendResponse) {
  // Check global flag to prevent multiple instances
  ...
  async function tryAutoFill() {
    ...
    if (inputs.length >= 4) {
      ...
      const result = await fillForm(inputs, data.username, data.password, data.fullname);

      if (result.username || result.password || result.fullname) {
        console.log('✅ Fill SUCCESS!');
        isCompleted = true; // Mark as completed
        window.__isAutoFilling = false; // Release global flag
    ...
    if (attempts < maxAttempts && !isCompleted) {
      ...
      console.error('❌ FAILED after', maxAttempts, 'attempts');
      showNotification('❌ Không tìm thấy form!');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'goToWithdraw') {
    ...
    const bankAccount = request.data?.bankAccount || '1234567890'; // Default for testing
    const bankName = request.data?.bankName || 'Vikki Bank'; // Default for testing

    goToWithdrawPage(withdrawPassword, bankAccount, bankName, (response) => {
      window.__isProcessingWithdraw = false; // Release flag when done
...
function goToWithdrawPage(withdrawPassword, bankAccount, bankName, sendResponse) {
  ...
  setTimeout(() => {
    ...
    for (let tab of tabs) {
      ...
      if (text === 'Tôi' || text.includes('Tôi')) {
        profileTab = tab;
        console.log('  ✅ Found "Tôi" tab');
        break;
      }
    }
